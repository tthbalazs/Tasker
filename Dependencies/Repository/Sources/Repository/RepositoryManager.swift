...
public final class RepositoryManager: RepositoryManagerInterface {
    private var cloudDatabaseManager: CloudDatabaseManagerInterface
    private var databaseManager: DatabaseManagerInterface

    public init(
        cloudDatabaseManager: CloudDatabaseManagerInterface,
        databaseManager: DatabaseManagerInterface
    ) {
        self.cloudDatabaseManager = cloudDatabaseManager
...
//        try await cloudDatabaseManager.getAll(parentObject: parentObject, objectsOfType: type)
        try await databaseManager.getAll(parentObject: parentObject)
    }
}
